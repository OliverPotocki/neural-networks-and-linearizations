...
def plot_signals(xs, t, labels=None, args=None, title=None, ratio=2.6, do_transform=False, dark_mode=False, zoom=1):
    rows, cols = 1, 10
...
def pad(x, padding, noise=(0,0)):
    low, high = padding
    pn, sn = noise
    x = dct(x, norm="ortho")
    p = low + int(np.random.rand()*(high-low+1))
    y = np.zeros((p)) + np.random.normal(loc=0.0, scale=pn, size=(p,))
    x = x + np.random.normal(loc=0.0, scale=sn, size=np.shape(x))
    return idct(np.concatenate([x, y]), norm="ortho")
...
def transform(x, y, args, eps=1e-8):
    new_x = pad(x+eps, args.padding, args.noise) # pad
...
def get_dataset_args(noise=(0,0), as_dict=False):
...
def gen_MNIST_1D_freq(noise):
    args = get_dataset_args(noise)
    set_seed(args.seed)
